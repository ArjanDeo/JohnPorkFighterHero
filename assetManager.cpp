...
const std::map<AssetManager::DirectoriesEnum, std::string> AssetManager::Directories = {
	...
	{AssetManager::DirectoriesEnum::SPRITES, "/sprites/"},
	{AssetManager::DirectoriesEnum::META, "/meta/"}
};
...
void AssetManager::loadTexture(const std::string& textureName) {
	...
	sf::Texture texture;
	if (!texture.loadFromFile(assetsDirectory + Directories.at(DirectoriesEnum::SPRITES) + textureName)) {
...
void AssetManager::loadFont(const std::string& fontName) {
	// Initialize the Font
	sf::Font font;
	if (!font.loadFromFile(assetsDirectory + Directories.at(DirectoriesEnum::META) + fontName)) {
